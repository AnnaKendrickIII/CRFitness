...
public class OrdersDAO implements OrdersDAO_interface {
	...
	public List<OrdersVO> findOrdersByMember_Id(String member_Id) {
		Query query = this
				.getSession()
				.createSQLQuery(
						"select * from Orders where Orders.Member_Id='"
								+ member_Id + "' and Order_Status!='作廢'")
				.addEntity(OrdersVO.class);
		return (List<OrdersVO>) query.list();
	}
	...
	public static void main(String[] args) {
		ApplicationContext context = new ClassPathXmlApplicationContext(
				"test.config.xml");
		OrdersDAO_interface dao = (OrdersDAO_interface) context
				.getBean("ordersDAO");
		...
		// List<OrdersVO> findOrdersByMember_Id =
		// dao.findOrdersByMember_Id("member1001");
